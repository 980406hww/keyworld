...
<%
	...
	List itemList = csm.getClientStatusSummary(datasourceName);
%>
	...
	<table width=50% style="font-size: 12px;" cellpadding=3>
		...
		<%
			...
			for (int i = 0; i < itemList.size(); i++) {
				ClientStatusSummaryVO value = (ClientStatusSummaryVO) itemList.get(i);
				trClass = "";
				validClient = true;
				keywordColor = "";
...
//				}
		...
		<tr <%=trClass%> >
			<%if(value.getClientIDPrefixCount() > 0){%>
			<td rowspan="<%=value.getClientIDPrefixCount()%>"><%=value.getClientIDPrefix()%></td>
			<td rowspan="<%=value.getClientIDPrefixCount()%>"><%=value.getClientIDPrefixTotalCount()%></td>
			<%}
			if(value.getTypeCount() > 0){
			%>
			<td rowspan="<%=value.getTypeCount()%>"><%=value.getType()%></td>
			<td rowspan="<%=value.getTypeCount()%>"><%=value.getTypeTotalCount()%></td>
			<%
				}
			%>
			<td><%=value.getCity()%></td>
