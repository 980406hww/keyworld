...
public class CustomerKeywordDailyReportSecondSummaryExcelWriter {
	...
	public CustomerKeywordDailyReportSecondSummaryExcelWriter(long dailyReportUuid, String externalAccount) throws BiffException, IOException {
		super();
  ...
  public byte[] getExcelContentBytes() throws Exception {
	...
	public void writeDailySummaryRow(Map<String, String> summaryMap, double percentage) throws ExcelWriteException{
		...
		if(day == endOfMonth){
			writer.addFormulanCell(CustomerKeywordDailyReportSummaryDefinition.TodayFee.getColumnIndex() + 1, day, "SUM(H2:H" + (day + 1) + ")*" + percentage);
		}
	}
