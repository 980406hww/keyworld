...
public class DownloadPositionHistoryLog extends HttpServlet {
	...
	public HttpServletResponse download(String fileName, String customerUuid, String customerName, String date, String statType, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			List<CustomerKeywordPositionHistoryLogView> views = manager.searchCustomerKeywordPositionHistoryLogViews("keyword", 100000 , 1 , customerUuid, customerName, date, statType, 1);
			if(!Utils.isEmpty(views)){
				KeywordPositionHistoryLogExcelWriter excelWriter = new KeywordPositionHistoryLogExcelWriter();
