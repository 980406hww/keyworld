...
public class MachineInfo {
    ...
    @TableField(value = "fCpuCount")
    private int cpuCount;

    @TableField(value = "fMemory")
    private int memory;
    ...
    public boolean getRed() {
        return (Utils.addMinutes(this.lastVisitTime,
            (10 > (this.getPageNo() * 3) ? (10 + 5) : (this.getPageNo() * 3 + 5)))
            .compareTo(Utils.getCurrentTimestamp()) < 0);
    }

    public boolean getYellow() {
        Timestamp time = Utils.addMinutes(this.lastVisitTime,
            (10 > (this.getPageNo() * 3) ? 10 : (this.getPageNo() * 3)));
        return time.compareTo(Utils.getCurrentTimestamp()) < 0
            && (Utils.addMinutes(time, 5)).compareTo(Utils.getCurrentTimestamp()) > 0;
    }
