...
VALUES ('前往行业列表', '/internal/industryList/toIndustryList', 'fi-thumbnails',
...
INSERT INTO t_role_resource(fRoleID, fResourceID) SELECT tem_role.fUuid, tem_resource.fUuid FROM (
		...
		(SELECT tr.fUuid FROM t_resource tr WHERE tr.fResourceName = '前往行业列表') tem_resource);