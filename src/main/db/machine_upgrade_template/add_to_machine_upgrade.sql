...
VALUES ('前往机器升级', '/internal/clientUpgrade/toClientUpgrades2', 'fi-thumbnails',
...
INSERT INTO t_role_resource(fRoleID, fResourceID) SELECT tem_role.fUuid, tem_resource.fUuid FROM (
		...
		(SELECT tr.fUuid FROM t_resource tr WHERE tr.fResourceName = '前往机器升级') tem_resource);