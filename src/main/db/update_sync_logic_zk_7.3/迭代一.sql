...
CREATE TABLE `cms_sync_manage` (
   ...
   `SYNC_OPERA_STATUS_TIME` varchar(20) DEFAULT NULL COMMENT '同步修改操作状态的时间',
   `SYNC_STATUS_TIME` varchar(20) DEFAULT NULL COMMENT '同步状态的时间',
   `SYNC_POSITION_TIME` varchar(20) DEFAULT NULL COMMENT '同步排名的时间',
   PRIMARY KEY (`ID`),
   UNIQUE KEY `USER_ID` (`USER_ID`,`COMPANY_CODE`,`TYPE`) USING BTREE,
   KEY `COMPANY_CODE` (`COMPANY_CODE`,`TYPE`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='同步管理表';
