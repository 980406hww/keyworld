...
VALUES ('前往负面清单', '/internal/negativelists/toSearchNegativeLists', 'fi-thumbnails',
...
INSERT INTO t_role_resource(fRoleID, fResourceID) SELECT tem_role.fUuid, tem_resource.fUuid FROM (
		...
		(SELECT tr.fUuid FROM t_resource tr WHERE tr.fResourceName = '前往负面清单') tem_resource);