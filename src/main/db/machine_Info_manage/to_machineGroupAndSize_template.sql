...
VALUES ('前往机器分组工作统计', '/internal/customerKeyword/toMachineGroupAndSize', 'fi-thumbnails',
...
INSERT INTO t_role_resource(fRoleID, fResourceID) SELECT tem_role.fUuid, tem_resource.fUuid FROM (
		...
		(SELECT tr.fUuid FROM t_resource tr WHERE tr.fResourceName = '前往机器分组工作统计') tem_resource);