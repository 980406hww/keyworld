...
VALUES ('前往操作类型管理', '/internal/operationTypes/toSearchOperationTypeLists', 'fi-thumbnails',
...
INSERT INTO t_role_resource(fRoleID, fResourceID) SELECT tem_role.fUuid, tem_resource.fUuid FROM (
		...
		(SELECT tr.fUuid FROM t_resource tr WHERE tr.fResourceName = '前往操作类型管理') tem_resource);